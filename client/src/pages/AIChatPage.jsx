        ...
        <div className="max-w-4xl mx-auto h-[calc(100vh-100px)] flex flex-col anime-fade-in md:pt-4">
            ...
            <div className="flex items-center justify-between px-4 pb-4 border-b border-white/5">
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-[1.2rem] grad-indigo flex items-center justify-center text-white shadow-lg shadow-theme">
                        <BrainCircuit className="w-7 h-7" />
                    </div>
                    <div>
                        <h1 className="text-xl font-black text-white tracking-tighter uppercase">AI Buddy</h1>
                        <p className="text-theme text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                ...
                {messages.map((msg, idx) => (
                    ...
                    >
                        <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-theme text-white' : 'bg-slate-800 text-theme border border-white/10'
                                }`}>
            ...
            <div className="px-4 py-2 flex gap-2 overflow-x-auto no-scrollbar whitespace-nowrap">
                {quickActions.map(action => (
                    <button
                        ...
                        className="px-4 py-2 glass rounded-full text-xs font-bold text-slate-400 hover:text-theme hover:border-theme transition-all border border-transparent"
                    >
                        {action}
                    </button>
                ))}
            ...
            <div className="p-4 pb-8 md:pb-4">
                <form
                    onSubmit={handleSend}
                    className="relative flex items-center bg-slate-900 border border-white/10 rounded-[2rem] p-2 focus-within:border-theme transition-all shadow-2xl"
                >
                    <input
                        type="text"
                        value={input}
                    ...
                    <button
                        ...
                        className="w-12 h-12 rounded-full grad-indigo flex items-center justify-center text-white shadow-lg shadow-theme active:scale-90 transition-all disabled:opacity-50"
                    >
